...
public final class ActivityUtils {
    ...
    /**
     * @purpose 判断事件出发时间间隔是否超过预定值(可用来防止连续点击,预定值设置为1000ms)
     * @param lastClickTime 最后一次点击时间,pretime设定的预定值
     * @return true前后两次间隔没超过预定值，false超过了
     */
	public boolean isFastDoubleClick(long lastClickTime,long pretime) {
		long time = System.currentTimeMillis();
		long timeD = time - lastClickTime;
		if (0 < timeD && timeD < pretime) {
			return true;
		}
		lastClickTime = time;
		return false;
